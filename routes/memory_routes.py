"""Memory-related API routes for the dashboard."""
...
from memory_utils import memory_usage_history, memory_usage_lock, log_memory_usage


def register_memory_routes(app, state_manager, get_active_connections, server_start_time):
    """Register memory management API routes."""
    ...
    def memory_profile():
        ...
        try:
            ...
            logging.error("Error in memory profiling: %s", exc)
            return jsonify({"error": str(exc)}), 500

    @app.route("/api/memory-history")
    def memory_history_api():
    ...
    def force_gc_api():
        ...
        try:
            ...
            log_memory_usage(state_manager, get_active_connections())
            return jsonify(
                {
                    "status": "success",
                    "collected": collected,
            ...
            logging.error("Error during forced GC: %s", exc)
